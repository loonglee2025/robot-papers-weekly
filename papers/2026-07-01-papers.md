...
**一句话总结**：FPL 通过自由形式的自然语言偏好轴和语言条件奖励模型，让机器人从人类多维度偏好中学习，无需重新训练即可在测试时调整行为。
...
**一句话总结**：Human-as-Humanoid 通过 ego-exo 视频和分段 IK 重定向，将大规模人类演示转换为可执行的 60-DoF 人形 VLA 训练数据，无需目标任务的机器人演示。
...
**一句话总结**：OOPSIEVERSE 是一个面向家庭操作的损伤感知仿真框架与基准，通过显式物理损伤信号支持更安全的演示采集、策略学习和 sim-to-real 迁移。
...
**一句话总结**：SARL 提出在语言提示空间中对通才机器人策略进行强化学习，通过调制预训练技能组合来解决零样本外分布的长程任务。
...
**一句话总结**：CoDex 是一个无需人类演示的零样本框架，利用 VLM 推断语义约束并生成候选功能抓取，通过强化学习精炼为可 sim-to-real 迁移的灵巧操作策略。
...
**一句话总结**：该文提出一种非线性铰接式拖拉机-拖车系统数学模型，并结合非线性模型预测控制显著改善路径跟踪中的拖车位置偏差。
...
**一句话总结**：Z-1 是一个面向 flow-based VLA 的强化学习后训练框架，使用 GRPO 和共享前缀 rollout 等技巧，在 24 个 RoboCasa 任务上平均成功率达到 80.6%。
...
**一句话总结**：RoboTacDex 是一个基于 Unitree G1 的多模态灵巧人形操作数据集，包含 6k 条轨迹、19 项任务和多视角 RGB-D、触觉反馈及语义标注。
...
**一句话总结**：DVG-WM 将世界建模解耦为动态学习与视觉合成，通过流匹配和潜在退化机制，在提升视频质量的同时将推理速度提高最高约 3.97 倍。
...
**一句话总结**：MVP-Nav 通过 3D 基础模型将单目 RGB 实例投影到 3D 有向包围盒，构建多层价值图，实现仅依赖 RGB 的物理感知零样本目标导航。
...
**一句话总结**：该研究提出无人机自主导航系统，通过 YOLOv11 检测和 DINOv2 姿态分类主动调整飞行以拍摄适合个体再识别的高质量侧腹图像，并在斑马、长颈鹿、老虎和大象上验证。
...
**一句话总结**：UniTacVLA 构建统一的触觉隐空间，通过触觉思维链推理和粗到细的未来触觉预测，增强 VLA 在接触丰富型灵巧操作中的理解与预测能力。
...
**一句话总结**：DynFly 提出动态感知的连续 B 样条轨迹生成框架，弥合高层 VLN 推理与可执行 UAV 运动之间的缺口，在 OpenUAV 基准上显著提升导航与轨迹质量。
...
**一句话总结**：该研究提出请求-响应窗口机制，将下一个云端请求点作为运动决策，在信号空间异构环境下保证机器人与云端大模型的可靠闭环执行。
...
**一句话总结**：该框架结合 LLM 偏好解释与 Apollo 规划参数集，将自然语言驾驶偏好映射为可执行的高速公路换道风格。
...
**一句话总结**：该研究将 BDDL 规范作为数据构建、计划验证和奖励设计的共享接口，配合 GroupAdapt 长度调度，使 8B 规划器在 BEHAVIOR-1000 上达到 97.3 Strict-Pass 并压缩响应长度 79%。
...
**一句话总结**：该研究为 Roller-skating 人形机器人训练并部署了强化学习控制策略，实现零样本从仿真到 Booster T1 硬件的迁移，展示动态平衡与高速运动能力。
...
**一句话总结**：FastDSAC 通过截断高斯分布近似策略并结合自适应方差调节，在并行采样场景下稳定训练、保持网络可塑性，并在 MuJoCo Playground 和 HumanoidBench 上实现更优收敛与渐近性能。
...
**一句话总结**：Labimus 是有机化学实验室人形灵巧操作的首个基准，重建 30 余个真实实验台资产，提出精度感知的评估协议揭示任务完成与实验有效性之间的差距。
...
**一句话总结**：KYON 是一款轮腿混合四足机器人，配备双臂上身，通过基座驱动和可重构小腿实现动态移动与灵巧操作。
...
**一句话总结**：WARP 提出离线人形全身重定向方法，通过 SEW 几何求解器和惰性移动基座控制，首次实现从离线人类演示到零样本全身移动操作的迁移。
...
**一句话总结**：AnyBody 允许部署时任意选择身体关键点来驱动统一的人形全身控制器，无需完整动作捕捉即可实现 loco-manipulation。
...
**一句话总结**：FADA 提出 Planner-IDM 三阶段小样本域适应框架，仅用约 2 分钟目标域 rollout 微调逆动力学模型，实现真实人形机器人在动力学变化下的高精度全身任务。
...
**一句话总结**：CWI 将全身动作捕捉数据解耦用于上半身操作和下半身 locomotion，通过 AMP 和多目标蒸馏，在 LimX Oli 上实现无需全身捕捉的 loco-manipulation 遥操作。
...
**一句话总结**：该研究在脚上嵌入低成本红外接近传感器提供“预接触”反馈，使四足机器人能够预测地形间断并稳健穿越离散地形。
...
**一句话总结**：ChronoFlow-Policy 通过稀疏 3D 关键点统一过去、当前和未来的交互流，并共同训练 ChronoFlow 表示与扩散动作序列，提升长程非马尔可夫操作任务的鲁棒性。
...
**一句话总结**：MuSe 通过多阶段融合、多感官未来预测和经验回放，将预训练视觉策略持续适应到力觉等新增模态，同时保持原任务性能。
...
**一句话总结**：该研究提出在高压缩比的层次化离散隐空间中直接搜索轨迹，统一了深度生成先验与基于搜索的运动规划，在 nuPlan 和 Waymo 上无需任务特定训练即实现强闭环规划性能。
...
**一句话总结**：该研究为四足机器人开发了软体柔性尾巴系统，可在杂乱/放射性环境中完成采样组织抓取、擦拭和释放，实现无需人类暴露的完整污染调查流程。
...
**一句话总结**：该方法将非抓握操作重新定义为优化以物体为中心的抓取性场，无需预设目标位姿即可实现闭环抓握准备。
...
**一句话总结**：BPP 通过单个人类演示作为行为提示，在测试时驱动机器人执行新任务，无需昂贵的微调。
...
**一句话总结**：ConCent 提出以接触事件序列为核心的 sim-to-real 强化学习框架，通过真实演示中的接触几何优化实现接触丰富任务的稳定迁移。
...
**一句话总结**：RRT-Rope 通过改进的 RRT-connect 快速生成可行路径，再用确定性 shortcutting 后处理，在大型地下采空区等 3D 环境中比现有算法快达 70%。
...
**一句话总结**：LeCropFollow 通过自监督语义热图提取与 TD-MPC2 在隐空间中直接规划轨迹，实现从简化仿真到真实农田的零样本转移，并显著减少种植间隙中的语义失败。
...
**一句话总结**：CRAFT 通过自监督失败发现、偏好评估和测试时解码重加权，缓解自回归交通仿真器从自车日志训练到全局闭环部署之间的上下文错位。
...
**一句话总结**：信息辅助标定（IAC）通过共享信息滤波框架，在有 GNSS 时提升 DVL 标定精度，在无 GNSS 时实现自标定，显著降低 AUV 导航漂移。
...
**一句话总结**：MOAR Planner 面向基础设施巡检 UAV，整合风险感知成本图、损伤/插入代价与自适应速度规划，实现安全、时间与能耗多目标实时路径优化。
...
**一句话总结**：AUSLUN 使用固定悬停无人机作为视觉-惯性里程计锚点，结合变焦吊舱扫描、测距定位与目标相对引导，为 GNSS 拒止海上环境提供搜索到导航的完整方案。
...
**一句话总结**：MyGO-Splat 将高斯原语解析为像素深度和法向，通过闭环几何监督相机位姿优化，并引入尺度自适应对齐，使单目 RGB SLAM 达到接近 RGB-D 方法的性能。
...
**一句话总结**：该研究基于 RBF 神经网络在线补偿四旋翼未建模动力学，结合 Lyapunov 稳定性导出的反馈线性化控制律，在 Gazebo 和真实 Crazyflie 2.1 上提升轨迹跟踪精度。
...
**一句话总结**：该框架通过解耦维度预测与扩散残差动力学精修，结合分布式非线性模型预测控制，实现无人机集群在复杂城市场景中的实时 4D 轨迹预测与跟踪。
